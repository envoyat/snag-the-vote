...
export const getPollingPlaceData = async (id: number, fetch: Function) => {
...
export const getDivisionDataByName = async (name: string, fetch: Function) => {
...
const getDivisionData = async (fetch: Function): Promise<Array<Divsion>> => {
  const headers = ['objectid', 'elect_div', 'area_sqkm', 'sortname', 'globalid', 'e_div_numb', 'numccds', 'actual', 'projected', 'total_popu', 'australian', 'Shape__Area', 'Shape__Length'];
  const fileName = 'Federal_Electoral_Boundaries.csv';
  var csvData = await loadCSVData<CSVDivision>(fileName, headers, fetch);
...
const getPollingPlacesData = async (fetch: Function): Promise<Array<PollingPlace>> => {
  const headers = ['State', 'DivisionID', 'DivisionNm', 'PollingPlaceID', 'PollingPlaceTypeID', 'PollingPlaceNm', 'PremisesNm', 'PremisesAddress1', 'PremisesAddress2', 'PremisesAddress3', 'PremisesSuburb', 'PremisesStateAb', 'PremisesPostCode', 'Latitude', 'Longitude'];
...
const getCandidateData = async (fetch: Function): Promise<Array<Candidate>> => {
  const headers = ['StateAb', 'DivisionID', 'DivisionNm', 'PartyAb', 'PartyNm', 'CandidateID', 'Surname', 'GivenNm', 'Elected', 'HistoricElected'];
  const fileName = 'HouseCandidatesDownload-27966.csv';
  var csvData = await loadCSVData<CSVCandidate>(fileName, headers, fetch);
  return csvData.map(d => ({
    surname: d.Surname,
    givenName: d.GivenNm,
    party: d.PartyNm,
...
const getMemberData = async (fetch: Function): Promise<Array<Member>> => {
  const headers = ['Honorific', 'Salutation', 'Post Nominals', 'Surname', 'First Name', 'Other Name', 'Preferred Name', 'Initials', 'Electorate', 'State', 'Political Party', 'Gender', 'Telephone', 'Electorate Address Line 1', 'Electorate Address Line 2', 'Electorate Suburb', 'Electorate State', 'Electorate PostCode', 'Electorate Telephone', 'Electorate Fax', 'Electorate TollFree', 'Electorate Postal Address', 'Electorate Postal Suburb', 'Electorate Postal State', 'Electorate Postal Postcode', 'Label Address', 'Label Suburb', 'Label State', 'Label Postcode', 'Parliamentary Title', 'Ministerial Title'];
  const fileName = 'FamilynameRepsCSV.csv';
  var csvData = await loadCSVData<CSVMember>(fileName, headers, fetch);
  return csvData.map(d => ({
    surname: d.Surname,
...
const loadCSVData = async <T>(fileName: string, headers: Array<string>, fetch: Function): Promise<Array<T>> => {
  const csvData = await fetch('/' + fileName);
  const csvText = await csvData.text();

  const csvRecords = parse(csvText, {
    columns: true,
